...
class system_settings extends MY_Controller
{
    ...
    function using_stock_desc($parent_id = NULL)
    {

        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
        $this->data['parent_id'] = $parent_id;
        $this->data['modal_js'] = $this->site->modal_js();
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => lang('using_stock_desc')));
        $meta = array('page_title' => lang('using_stock_desc'), 'bc' => $bc);
        $this->page_construct('settings/using_stock_desc', $meta, $this->data);
    }

    function add_using_stock_desc($parent_id = NULL)
    {
        $this->load->helper('security');
        $this->form_validation->set_rules('code', lang("desc_code"), 'trim|is_unique[position.code]|required');
        $this->form_validation->set_rules('name', lang("desc_name"), 'required');
        ...
        } elseif ($this->input->post('add_desc')) {
            $this->session->set_flashdata('error', validation_errors());
            redirect($_SERVER['HTTP_REFERER']);
        }

        if ($this->form_validation->run() == true && $cid = $this->settings_model->addUsingStockDesc($data)) {
            $this->session->set_flashdata('message', lang("desc_added"));
            redirect($_SERVER["HTTP_REFERER"]);
        } else {
            $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
            ...
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/add_using_stock_desc', $this->data);
        }
    }

    function edit_using_stock_desc($id = NULL)
    {
        $this->load->helper('security');
        $this->form_validation->set_rules('code', lang("category_code"), 'trim|required');
        $desc_details = $this->settings_model->getStockUsingDescByID($id);
        if ($this->input->post('code') != $desc_details->code) {
            $this->form_validation->set_rules('code', lang("category_code"), 'is_unique[categories.code]');
        }
        $this->form_validation->set_rules('name', lang("category_name"), 'required|min_length[3]');
        ...
        } elseif ($this->input->post('edit_desc')) {
            $this->session->set_flashdata('error', validation_errors());
            redirect($_SERVER['HTTP_REFERER']);
        }

        if ($this->form_validation->run() == true && $this->settings_model->updateStockUsingDesc($id, $data)) {
            $this->session->set_flashdata('message', lang("desc_updated"));
            redirect($_SERVER['HTTP_REFERER']);
        } else {
            $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
            $description = $this->settings_model->getStockUsingDescByID($id);
            ...
            $this->data['name'] = array('name' => 'name',
                ...
                'class' => 'form-control',
                'required' => 'required',
                'value' => $this->form_validation->set_value('name', $description->name),
            );
            $this->data['code'] = array('name' => 'code',
                'id' => 'code',
                'type' => 'text',
                'class' => 'form-control',
                'required' => 'required',
                'value' => $this->form_validation->set_value('code', $description->code),
            );
            
            $this->data['setting'] = $this->site->get_setting();
            $this->data['modal_js'] = $this->site->modal_js();
            $this->data['id'] = $id;
            $this->load->view($this->theme . 'settings/edit_using_stock_desc', $this->data);
        }
    }

    function delete_using_stock_desc($id = NULL)
    {
        if ($this->settings_model->deleteStockUsingDesc($id)) {
            $this->session->set_flashdata('message', lang("desc_deleted"));
            redirect("system_settings/using_stock_desc");
        }
    }
